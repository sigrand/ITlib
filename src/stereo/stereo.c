                    ...
                    const int f, const int d, const int sl, const int md, const int maxd, const int mind)
{
    ...
    int16 *l[ls], *r[ls], *le, *re, *tm, *inl, *inr, *tmp, *lin;

    int max = maxd>>8, min = mind>>8, dis;
    int ninr, ninl, lmax, mp=0, lbeg, xbl;
    double df = (double)d*(double)f/(double)max;
    ...
    for(y=0; y < h; y++){
        ...
        cp_line_16(&ledg[yw], le, w, sh);
        cp_line_16(&redg[yw], re, w, sh);

        seg_horiz_max(re, le, inr, inl, w, sh, sl, &ninr, &ninl);
        for(i=0; i < w; i++) lin[i] = 0;
        ...
        //for(x=0; x < w-sl; x++){
            ...
            //if(re[sh][x+sh]){
                ...
                */
                ...
                lbeg = x+sl;
                lmax = x+sl+md >= w ? w : x+sl+md;
                //printf("inr = %d lbeg = %d lmax = %d\n", x , lbeg, lmax);
                while(1){
                    if (k >= ninl) {
                         break;
                    }
                    if(inl[k] >= lbeg && inl[k] < lmax) {
                        m = k;
                ...
                if(sad){
                    ...
                    if(sadt < ths){
                        ...
                        if(dis > 255) {
                            ...
                            //Check if left position was before
                            if(!lin[bl]){
                                lin[bl] = xbl;
                                out[yw + xbl] = dis;
                                gp++;
                            ...
                            //printf("y = %d xl = %d xr = %d out = %d sad = %d, el = %d er = %d\n",
                            //       y, bl, x, out[yw + ((x + bl)>>1)], sadt, le[bl+sh], re[x+sh]);
    ...
    printf("Total pixels = %d good pixele = %d  maching  = %d %d \n", tp, gp, mp, mp*100/(w*h));
...
void stereo_filter(const int16 *in, int16 *out, int16 *buff, const int w, const int h)
{
    ...
    int sh = 2, ls = (sh<<1)+1, h1 = h-sh, w2 = w + (sh<<1), w1 = w*sh, sq = ls*ls;
    int16 *d[ls], *tm;
